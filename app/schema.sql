DROP TABLE IF EXISTS UserProjects CASCADE;
DROP TABLE IF EXISTS Users CASCADE;
DROP TABLE IF EXISTS Record CASCADE;
DROP TABLE IF EXISTS Project CASCADE;
DROP TABLE IF EXISTS Coastal6;

CREATE TABLE Users
(
    user_id   INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    email     VARCHAR(255) NOT NULL UNIQUE,
    firstname VARCHAR(255) NOT NULL,
    lastname  VARCHAR(255),
    role      VARCHAR(255) NOT NULL,
    password  VARCHAR(255) NOT NULL,
    inactive  BOOLEAN DEFAULT FALSE
);


CREATE TABLE Project
(
    project_id   INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created      TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    project_name VARCHAR(255) NOT NULL UNIQUE,
    code         CHAR(2)      NOT NULL,
    finished     BOOLEAN
);

CREATE TABLE UserProjects
(
    user_id    INT,
    project_id INT,
    FOREIGN KEY (user_id) REFERENCES Users (user_id) ON DELETE CASCADE,
    FOREIGN KEY (project_id) REFERENCES Project (project_id) ON DELETE CASCADE,
    PRIMARY KEY (user_id, project_id)
);

CREATE TABLE Record
(
    record_id          INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    creator_id         INTEGER      NOT NULL,
    project_id_1       INT,
    project_id_2       INT,
    created            TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    record_name        VARCHAR(255) NOT NULL UNIQUE,
    data_location_type VARCHAR(255) NOT NULL,
    data_location      VARCHAR(255) NOT NULL,
    invenio            BOOLEAN,
    record_description   VARCHAR(255) NOT NULL,
    uid                VARCHAR(18)  NOT NULL UNIQUE,
    FOREIGN KEY (creator_id) REFERENCES Users (user_id),
    FOREIGN KEY (project_id_1) REFERENCES Project (project_id),
    FOREIGN KEY (project_id_2) REFERENCES Project (project_id)
);

CREATE TABLE Coastal6
(
    reference_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    record_id      INT NOT NULL,
    FOREIGN KEY (record_id) REFERENCES Record (record_id) ON DELETE CASCADE
);

INSERT INTO public.project(project_id, created, project_name, code)
VALUES (0, '2023-08-08 11:02:29', '', 'XX');

INSERT INTO public.project(project_id, created, project_name, code, finished)
VALUES (1, '2023-08-08 11:02:29', 'General Record', 'GD', False);

INSERT INTO public.project(project_id, created, project_name, code, finished)
VALUES (2, '2023-08-08 11:02:29', 'test', 'TS', False);

INSERT INTO public.project(project_id, created, project_name, code, finished)
VALUES (3, '2023-08-08 12:00:00', 'asdf', 'AS', False);
